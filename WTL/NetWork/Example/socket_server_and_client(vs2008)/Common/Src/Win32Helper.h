...
public:
	...
	: m_bIsFirstApp(FALSE)
	{
		m_hMap = ::CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 4, pszAppFlag);
	...
	~COnlyOneApp() {if(m_hMap) {::CloseHandle(m_hMap); m_hMap = NULL;}}
...
{
public:
	BOOL IsFirstApp()			{return m_hwndPre == NULL;}
	HWND GetPreInstanceWindow()	{return m_hwndPre;}

	COnlyOneWndApp(LPCTSTR lpszClassName, LPCTSTR lpszWindowName = NULL)
	{
...
{
public:
	auto_dc(HDC h = NULL, HWND w = NULL, bool is_create = false)
	{
		set(h, w, is_create);
	}
	...
	auto_dc& reset(HDC h = NULL, HWND w = NULL, bool is_create = false)
	{
		if(h != m_h || w != m_w)
		{
			if(is_valid())
	...
	*/
	...
	bool		is_valid	()	const	{return m_h != NULL;}
...
public:
	...
	bool is_valid()	const	{return m_hdc != NULL;}
...
{
public:
	auto_gdi_obj(T obj = NULL)				{set(obj);}
	auto_gdi_obj(auto_gdi_obj<T>& other)	{set(other.release());}

	~auto_gdi_obj()	{reset();}

	auto_gdi_obj<T>& reset(T obj = NULL)
	{
		if(obj != m_obj)
		{
			if(is_valid())
	...
	bool		is_valid()	const	{return m_obj != NULL;}
...
{
public:
	auto_bitmap(HBITMAP obj = NULL) : auto_bitmap_base(obj) {}
...
{
public:
	auto_brush(HBRUSH obj = NULL) : auto_brush_base(obj) {}
...
{
public:
	auto_palette(HPALETTE obj = NULL) : auto_palette_base(obj) {}
...
{
public:
	auto_pen(HPEN obj = NULL) : auto_pen_base(obj) {}
...
{
public:
	auto_rgn(HRGN obj = NULL) : auto_rgn_base(obj) {}
...
{
public:
	auto_font(HFONT obj = NULL) : auto_font_base(obj) {}
