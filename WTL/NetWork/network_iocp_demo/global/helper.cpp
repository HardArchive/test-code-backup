...
#include "../common/com_module/FuncHelper.h"



HWND g_hWndPostMessage = NULL;
...
void PostOnSend(DWORD dwConnectionID, const BYTE* pData, int iLength)
{
	...
	*/
	...
	LPTSTR lpszContent = new TCHAR[2048];
	wsprintf(lpszContent, _T("%s"), pData);
	int content_len = iLength;
	info_msg* msg = info_msg::Construct(dwConnectionID, EVT_ON_SEND, content_len, lpszContent);
...
{
	if(g_hWndPostMessage == NULL || !::PostMessage(g_hWndPostMessage, USER_INFO_MSG, (WPARAM)msg, NULL))
