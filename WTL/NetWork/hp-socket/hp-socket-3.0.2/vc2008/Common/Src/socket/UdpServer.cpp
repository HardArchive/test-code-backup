...
BOOL CUdpServer::CheckParams(BOOL bPreconditions)
{
	if(bPreconditions)
...
{
	TUdpSocketObj*	pSocketObj	= (TUdpSocketObj*)m_phSocket.Alloc(sizeof(TUdpSocketObj) + sizeof(CRITICAL_SECTION), HEAP_ZERO_MEMORY);
	CRITICAL_SECTION* pcrisec	= (CRITICAL_SECTION*)(((char*)pSocketObj) + sizeof(TUdpSocketObj));
...
BOOL CUdpServer::GetConnectionCriSec(CONNID dwConnID, CCriSec2** ppCriSec)
{
	ASSERT(ppCriSec != nullptr);

	TUdpSocketObj* pSocketObj = FindSocketObj(dwConnID);

	if(pSocketObj != nullptr)
	{
		*ppCriSec = &(pSocketObj->crisec);
		return TRUE;
	}
