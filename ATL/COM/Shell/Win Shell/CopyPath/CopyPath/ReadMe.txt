...
应用程序向导为您创建了这个 CopyPath 项目，供您用作
编写动态链接库(DLL)的起点。
...
CopyPath.idl
    ...
    此文件将由 MIDL 编译器处理以生成:
        C++ 接口定义和 GUID 声明(CopyPath.h)
        GUID 定义(CopyPath_i.c)
        类型库(CopyPath.tlb)
        封送处理代码(CopyPath_p.c and dlldata.c)

CopyPath.h
    此文件包含在 CopyPath.idl 中定义的项的 C++
...
CopyPathps.vcproj
	...
	dlldata.c、CopyPath_i.c 和 CopyPath_p.c，这些是
