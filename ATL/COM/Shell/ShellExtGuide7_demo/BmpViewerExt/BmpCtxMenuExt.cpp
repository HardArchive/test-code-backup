    ...
    UINT uidCmdLast, UINT uFlags )
{
    ...
    // Get the version of the shell.  If it's > 4.0, we can use an owner-drawn 
    // item.
    ...
    if ( NULL != hinstShell )
        ...
        if ( NULL != pProc )
            ...
            if ( SUCCEEDED( pProc ( &rInfo ) ))
                {
                // Check for a major version greater than 4.0 (Win 2K or later),
                // or minor version >= 71 (95/NT4 with Active Desktop).
                if ( rInfo.dwMajorVersion > 4 ||
                    (rInfo.dwMajorVersion == 4 && rInfo.dwMinorVersion >= 71) )
                    {
                    bUseOwnerDraw = true;
                    }
                }
...
STDMETHODIMP CBmpCtxMenuExt::GetCommandString (
    UINT uCmd, UINT uFlags, UINT* puReserved, LPSTR pszName, UINT cchMax )
{
USES_CONVERSION;
