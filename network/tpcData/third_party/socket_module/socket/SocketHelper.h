...
public:
	//接收连接通知
	virtual EnHandleResult OnAccept(DWORD dwConnectionID, DWORD dwIPValue)	= 0;
	//服务关闭通知
...
struct TSocketObj : public TSocketObjBase
{
	...
	////取得IP字符串
	//inline void GetIP(string& strIP)
	//{
	//	strIP = string(inet_ntoa(clientAddr.sin_addr));
	//}

	//取得IP字符串
	inline DWORD GetIPValue()
	{
		//addr.sin_addr.s_addr = dwIPValue;  
