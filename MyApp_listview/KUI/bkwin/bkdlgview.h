...
// 					m_bCanMaximize = TRUE;
	...
	// 根据ID找对应的对象指针，从Header-->Body-->Footer依次找
...
#define BORDER_SIZE		5
	LRESULT OnNcHitTest(CPoint point)
	{
		...
		if (0 != (wndParent.GetStyle() & (WS_MAXIMIZE)))
		{
			wndParent.SetWindowLongPtr(DWL_MSGRESULT, HTCLIENT);
			return HTCLIENT;
		}

		wndParent.GetClientRect(&rect);
		wndParent.ScreenToClient(&point);
		x = point.x;  y = point.y;

		if(x <= rect.left + BORDER_SIZE) {
			if( y <= rect.top + BORDER_SIZE) {
				nHitTest = HTTOPLEFT;
				goto HitTestExit;
			}
			if(y >= rect.bottom - BORDER_SIZE) {
	...
	HitTestExit:
		wndParent.SetWindowLongPtr(DWL_MSGRESULT, nHitTest);
		return nHitTest;
...
*/
	void OnNCLButtonDown(UINT nHitTest, CPoint point)
	{
		...
		case HTCAPTION:
				...
				break;
		case HTTOP:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_TOP);
			break;
		case HTBOTTOM:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_BOTTOM);
			break;
		case HTLEFT:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_LEFT);
			break;
		case HTRIGHT:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_RIGHT);
			break;
		case HTTOPLEFT:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_TOPLEFT);
			break;
		case HTTOPRIGHT:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_TOPRIGHT);
			break;
		case HTBOTTOMLEFT:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_BOTTOMLEFT);
			break;
		case HTBOTTOMRIGHT:
			wndParent.SendMessage(WM_SYSCOMMAND, SC_SIZE | WMSZ_BOTTOMRIGHT);
			break;
		default:
			break;
		}
    ...
    void OnLButtonDown(UINT nFlags, CPoint point)
    {
        ...
        if (hBkWndHitTest)
        {
            ...
            if (!(pWndPushDown->IsClass("button") || pWndPushDown->IsClass("imgbtn") || pWndPushDown->IsClass("link")))
            {
                ...
                if (0 == (wndParent.GetStyle() & (WS_MAXIMIZE | WS_MINIMIZE)))
				{
                    wndParent.SendMessage(WM_NCLBUTTONDOWN, HTCAPTION);
                    //wndParent.SendMessage(WM_SYSCOMMAND, SC_MOVE | HTCAPTION);
				}
    ...
    BOOL OnSetCursor(CWindow /*wnd*/, UINT nHitTest, UINT message)
    {
		...
		if((HTCAPTION==nHitTest) || (HTSYSMENU==nHitTest) ||(HTMENU==nHitTest) ||(HTCLIENT==nHitTest))
		{
			::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE(IDC_ARROW)));
		}
		...
		else if((HTLEFT==nHitTest) || (HTRIGHT==nHitTest))
		{
...
protected:
    BEGIN_MSG_MAP_EX(CBkDialogViewImpl)
		...
		MSG_WM_NCHITTEST(OnNcHitTest) // added by wilson
		MSG_WM_NCLBUTTONDOWN(OnNCLButtonDown) // added by wilson
