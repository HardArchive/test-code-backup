...
#define FireAccept(id, ipvalue)			(HasStarted() ? m_psoListener->OnAccept(id, ipvalue)	: ISocketListener::HR_IGNORE)
...
//在C++中，类对静态成员的构造和初始化仅有部分规定，甚至每次编译都有可能不同，
//这很容易导致难以察觉的缺陷。因此，除了不允许定义全局类变量外，我们也不允许使用一
//个函数来初始化一个全局变量，除非这个函数（比如geteny(),getpid()）不依赖任何
//Google C++ Style Guide
//const DWORD	CIocpServer::DEFAULT_IOCP_THREAD_COUNT		= ::GetCpuCount() * 2 + 2;  //此种初始化方式有问题
...
BOOL CIocpServer::StartAcceptThread()
{
	...
	//将上一步建立的HANDLE(比如socket）绑定到第一步得到的IO Completion Port的HANDLE 上
...
BOOL CIocpServer::Disconnect(DWORD dwConnID)
{
	...
	if(NULL != pSocketObj && pSocketObj->socket != INVALID_SOCKET)
		return //shutdown(pSocketObj->socket, SD_SEND) != SOCKET_ERROR;
	WSANOTINITIALISED == closesocket(pSocketObj->socket);
...
UINT WINAPI CIocpServer::AcceptThreadProc(LPVOID pv)
{
	...
	TRACE(_T("---------------> Accept Thread 0x%08X started <---------------\r\n"), ::GetCurrentThreadId());
	...
	while(TRUE)
	{
		...
		*/
		...
		{//第一个参数发出了信号
			...
			TRACE(_T("---------------> Accept Thread 0x%08X stoped <---------------\r\n"), ::GetCurrentThreadId());
			break;
		}
		else
			VERIFY(FALSE);
		...
		if (!result)
		{
			...
			if(pServer->HasStarted())
			{
				...
				if (!result)
				{
					...
					TRACE(_T("GetQueuedCompletionStatus failed (SYS: %d, SOCK: %d, FLAG: %d)\r\n"), dwSysCode, dwErrorCode, dwFlag);
				}
			}
			else   
...
void CIocpServer::HandleAccept(SOCKET soListen, TBufferObj* pBufferObj)
{
	...
	m_cslCriSecConnID.Lock();
	m_dwConnID++;
	pSocketObj->connID = m_dwConnID;
	m_cslCriSecConnID.Unlock();
	...
	FireAccept(pSocketObj->connID, pSocketObj->GetIPValue());  //处理连接
	//接受连接成功后 投递一个接收
...
void CIocpServer::HandleReceive(TSocketObj* pSocketObj, TBufferObj* pBufferObj)
{
	...
	{
		TRACE(_T("<CNNID: %d> FireReceive() return HR_ERROR, connection will be closed\r\n"), pSocketObj->connID);
