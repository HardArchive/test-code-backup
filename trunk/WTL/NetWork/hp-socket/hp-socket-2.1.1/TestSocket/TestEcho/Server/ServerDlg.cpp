...
void CServerDlg::OnBnClickedDisconnect()
{
	...
	DWORD dwConnID = (DWORD)_ttoi(strConnID);
...
ISocketListener::EnHandleResult CServerDlg::OnAccept(DWORD dwConnectionID, SOCKET soClient)
{
	BOOL bPass = TRUE;
	CString strAddress;
	USHORT usPort;

	m_Server.GetClientAddress(dwConnectionID, strAddress, usPort);
	...
	::PostOnAccept(dwConnectionID, strAddress, usPort, bPass);
