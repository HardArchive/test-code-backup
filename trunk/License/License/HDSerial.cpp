...
char* CGetHDSerial::GetHDSerial()
{
	...
	else
	{
		...
		// 判断是否有SCSI硬盘
		if ( ! WinNTReadIDEHDSerial(nHARD_MODE, m_wWinNTHDSerial)) 
		{
			//AfxMessageBox("获取IDE失败,开始获取SCSI");
			if (!WinNTReadSCSIHDSerial(nHARD_MODE, m_wWinNTHDSerial))
			{
				return NULL;
			}
...
//Mod by lichunhui 20061229 for master&slave drive hard
BOOL CGetHDSerial::WinNTReadIDEHDSerial (int drive, DWORD * buffer) // Windows NT/2000/XP下读取IDE硬盘序列号
{
...
//Mod by lichunhui 20061229 for master&slave drive hard
BOOL CGetHDSerial::WinNTReadSCSIHDSerial(int controller, DWORD * pBuffer)   //WindowsNT/2000/XP系统下读取SCSI硬盘序列号
{
	pBuffer[0]='\n';
	BOOL bFlag = FALSE;
		...
		for (drive = 0; drive < 2; drive++)
		{
				...
				&dummy, NULL))
			{
				...
				if (pId -> sModelNumber [0])
				{
					...
					for (n = 0; n < 256; n++)
						pBuffer[n] = pIdSector [n];  //Mod by lichunhui 20070111

					CloseHandle (hScsiDriveIOCTL); //Add by lichunhui 20070111
					return TRUE;  // 读取成功                               
				}
			}
		}
...
int CGetHDSerial::GetSysDiskNum()
{

	HANDLE hDevice; DWORD bytesReturned; char rawDiskName[] ="\\\\.\\c:";
	//假定是c分区 
	hDevice = CreateFile(rawDiskName, GENERIC_READ, FILE_SHARE_READ | 
		FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (hDevice == INVALID_HANDLE_VALUE) 
		return FALSE; 
	STORAGE_DEVICE_NUMBER deviceInfo;
	BOOL retcode=DeviceIoControl(hDevice, IOCTL_STORAGE_GET_DEVICE_NUMBER, NULL, 0,
		&deviceInfo, sizeof(deviceInfo), &bytesReturned, NULL); 
	if(retcode==FALSE) 
	{ 
		CloseHandle(hDevice); 
		return 0;
	...
	return deviceInfo.DeviceNumber;//就是第几颗。 
}
