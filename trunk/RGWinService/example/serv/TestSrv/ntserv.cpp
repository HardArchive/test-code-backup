...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnHardwareProfileChange was called, EventType:%08X\n"), dwEventType);
#endif
...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnPowerEvent was called, EventType:%08X\n"), dwEventType);
#endif
...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnDeviceEvent was called, EventType:%08X\n"), dwEventType);
#endif
...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnSessionChange was called, EventType:%08X\n"), dwEventType);
#endif
}
...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnPreShutdown was called\n"));
#endif
}
...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnTimeChange was called\n"));
#endif
}
...
{
#ifdef _DEBUG
  TRACE(_T("CNTService::OnTriggerEvent was called\n"));
#endif
}
...
BOOL CNTService::WriteServiceProfileStringArray(LPCTSTR lpszService, LPCTSTR lpszSection, LPCTSTR lpszEntry, const CStringArray& array, BOOL bFlush, DWORD* pLastError)
{
  //Validate our parameters
	ASSERT(lpszSection != NULL);
  ASSERT(lpszService != NULL);
...
CString CNTService::GetServiceProfileString(LPCTSTR lpszService, LPCTSTR lpszSection, LPCTSTR lpszEntry, LPCTSTR lpszDefault, DWORD* pLastError)
{
  //Validate our parameters
  ASSERT(lpszService != NULL);
  ASSERT(lpszSection != NULL);
...
BOOL CNTService::GetServiceProfileStringArray(LPCTSTR lpszService, LPCTSTR lpszSection, LPCTSTR lpszEntry, CStringArray& array, DWORD* pLastError)
{
  //Validate our parameters
  ASSERT(lpszService != NULL);
	ASSERT(lpszSection != NULL);
...
BOOL CNTService::GetServiceProfileBinary(LPCTSTR lpszService, LPCTSTR lpszSection, LPCTSTR lpszEntry, LPBYTE* ppData, ULONG* pnBytes, DWORD* pLastError)
{
  ...
  ASSERT(lpszService != NULL);
	...
	AFXASSUME(ppData != NULL);
	AFXASSUME(pnBytes != NULL);
...
{
  //Validate our parameters
  AFXASSUME(lpszServiceName != NULL);
...
BOOL CNTService::Run()
{
  ...
  _tcscpy_s(pszServiceName, sizeof(pszServiceName)/sizeof(TCHAR), m_sServiceName);
...
BOOL CNTService::GetDependencies(ATL::CHeapPtr<TCHAR>& mszDependencies)
{
  ...
  for (INT_PTR i=0; i<nDependencies; i++)
    dwSize += (m_sDependencies.GetAt(i).GetLength() + 1);
    
  //Need one second NULL for the double NULL at the end
  dwSize++;

  //Now form the string
  if (!mszDependencies.Allocate(dwSize))
  {
...
void CNTService::SecureEmptyString(CString& sVal)
{
...
BOOL CNTService::InstallEventLogSource(CString& sErrorMsg, DWORD& dwError)
{
  //Validate our parameters
  ASSERT(m_bEventLogSource);
...
BOOL CNTService::InstallServiceConfiguration(CNTScmService& service, CString& sErrorMsg, DWORD& dwError)
{
...
BOOL CALLBACK CNTService::_EnumerateInstances(void* pData, ENUM_SERVICE_STATUS& ss)
{
  ...
  AFXASSUME(pInfo);
  AFXASSUME(pInfo->pManager);
  AFXASSUME(pInfo->psAppPath);
  AFXASSUME(pInfo->pServiceNames);
...
BOOL CNTService::Uninstall(CString& sErrorMsg, DWORD& dwError, DWORD dwTimeToWaitForStop)
{
  ...
  if (!manager.Open(NULL, SC_MANAGER_CONNECT))
  {
    ...
    if (m_bEventLogSource)
    {
      CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_UNINSTALL_FAIL_OPEN_SCM), dwError, TRUE));
  ...
  if (!manager.OpenService(m_sServiceName, DELETE | SERVICE_STOP | SERVICE_QUERY_STATUS, service))
  {
    ...
    if (m_bEventLogSource)
    {
      CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_UNINSTALL_FAIL_OPEN_SERVICE), dwError, TRUE));
  ...
  if (!service.Delete())
  {
    ...
    if (m_bEventLogSource)
    {
      CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_UNINSTALL_FAIL_DELETE_SERVICE), dwError, TRUE));
...
BOOL CNTService::SetServiceToStatus(ServiceAction esaStatusWanted, DWORD& dwError, DWORD dwTimeout, DWORD dwPollingInterval)
{
  ...
  {
    dwError = GetLastError();
    CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_OPEN_SCM), dwError, FALSE));
    TRACE(_T("%s\n"), sError.operator LPCTSTR());
    sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_OPEN_SCM), dwError, TRUE);
    if (m_bEventLogSource)
      m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_CONNECT_SCM), sError);
    return FALSE;
  }
  ...
  {
    dwError = GetLastError();
    CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_OPEN_SERVICE), dwError, FALSE));
    TRACE(_T("%s\n"), sError.operator LPCTSTR());
    sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_OPEN_SERVICE), dwError, TRUE);
    if (m_bEventLogSource)
      m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_OPEN_SERVICE), sError);
    return FALSE;
  }
  ...
  if (service.QueryStatus(status))
  {
    if (status.dwCurrentState != dwStatusWanted)
    {
      ...
      switch (esaStatusWanted) 
      {
        case ActionStartService:
        {
          ...
          {
            dwError = GetLastError();
            CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_START), dwError, FALSE));
            TRACE(_T("%s\n"), sError.operator LPCTSTR());
            sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_START), dwError, TRUE);
            if (m_bEventLogSource)
              m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_START), sError);
          }
          break;
        }
        case ActionPauseService:
        {
          ...
          {
            dwError = GetLastError();
            CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_PAUSE), dwError, FALSE));
            TRACE(_T("%s\n"), sError.operator LPCTSTR());
            sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_PAUSE), dwError, TRUE);
            if (m_bEventLogSource)
              m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_PAUSE), sError);
          }
          break;
        }
        case ActionContinueService:
        {
          ...
          {
            dwError = GetLastError();
            CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_CONTINUE), dwError, FALSE));
            TRACE(_T("%s\n"), sError.operator LPCTSTR());
            sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_CONTINUE), dwError, TRUE);
            if (m_bEventLogSource)
              m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_CONTINUE), sError);
          }
          break;
        }
        case ActionStopService:
        {
          ...
          {
            dwError = GetLastError();
            CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_STOP), dwError, FALSE));
            TRACE(_T("%s\n"), sError.operator LPCTSTR());
            sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_STOP), dwError, TRUE);
            if (m_bEventLogSource)
              m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_STOP), dwError);
          }
          break;
        }
        default:
    ...
    {
      dwError = GetLastError();
      CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_WAITFORCHANGE), dwError, FALSE));
      TRACE(_T("%s\n"), sError.operator LPCTSTR());
      sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_WAITFORCHANGE), dwError, TRUE);
      if (m_bEventLogSource)
        m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_CHANGE_STATE), sError, dwStatusWanted, FALSE);
  ...
  {
    dwError = GetLastError();
    CString sError(Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_QUERYSTATUS), dwError, FALSE));
    TRACE(_T("%s\n"), sError.operator LPCTSTR());
    sError = Win32ErrorToString(MapResourceID(IDS_NTSERV_SETSERVICETOSTATUS_FAIL_QUERYSTATUS), dwError, TRUE);
    if (m_bEventLogSource)
      m_EventLogSource.Report(EVENTLOG_ERROR_TYPE, MapMessageID(CNTS_MSG_SERVICE_FAIL_QUERY_STATUS), sError);
    return FALSE;
  }
...
DWORD CNTService::ProcessShellCommand(CNTServiceCommandLineInfo& rCmdInfo)
{
  ...
  switch (rCmdInfo.m_nShellCommand)
  {
    ...
    case CNTServiceCommandLineInfo::StartTheService:
    {
      ...
      {
        CString sErrorMsg(Win32ErrorToString(MapResourceID(IDS_NTSERV_FAIL_START_SERVICE), dwError, FALSE));
    ...
    case CNTServiceCommandLineInfo::PauseService:
    {
      ...
      {
        CString sErrorMsg(Win32ErrorToString(MapResourceID(IDS_NTSERV_FAIL_PAUSE_SERVICE), dwError, FALSE));
        if (!rCmdInfo.m_bSilent)
        {
          if (m_bUseConsole)
            _tprintf(_T("%s\n"), sErrorMsg.operator LPCTSTR());
    ...
    case CNTServiceCommandLineInfo::ContinueService:
    {
      ...
      {
        CString sErrorMsg(Win32ErrorToString(MapResourceID(IDS_NTSERV_FAIL_CONTINUE_SERVICE), dwError, FALSE));
        if (!rCmdInfo.m_bSilent)
        {
          if (m_bUseConsole)
            _tprintf(_T("%s\n"), sErrorMsg.operator LPCTSTR());
    ...
    case CNTServiceCommandLineInfo::StopService:
    {
      ...
      {
        CString sErrorMsg(Win32ErrorToString(MapResourceID(IDS_NTSERV_FAIL_STOP_SERVICE), dwError, FALSE));
        if (!rCmdInfo.m_bSilent)
        {
          if (m_bUseConsole)
            _tprintf(_T("%s\n"), sErrorMsg.operator LPCTSTR());
    ...
    case CNTServiceCommandLineInfo::InstallService:
    {
      ...
      {
        CString sName;
        sName.Format(_T(" \"/SN:%s\""), rCmdInfo.m_sServiceName.operator LPCTSTR());
        m_sBinaryPathName += sName;
      }
      if (m_bAllowDisplayNameChange && rCmdInfo.m_sServiceDisplayName.GetLength())
      {
        CString sDisplayName;
        sDisplayName.Format(_T(" \"/SDN:%s\""), rCmdInfo.m_sServiceDisplayName.operator LPCTSTR());
        m_sBinaryPathName += sDisplayName;
      }
      if (m_bAllowDescriptionChange && rCmdInfo.m_sServiceDescription.GetLength())
      {
        CString sDescription;
        sDescription.Format(_T(" \"/SD:%s\""), rCmdInfo.m_sServiceDescription.operator LPCTSTR());
...
CString CNTService::GetErrorMessage(DWORD dwError)
{
  //What will be the return value from this function
  CString sError;
...
CString CNTService::Win32ErrorToString(UINT nStringID, DWORD dwError, BOOL bEventLog)
{
  ...
  if (bEventLog)
  {
    switch (m_ELLoggingDetail)
    {
      ...
      {
        CString sDescription(GetErrorMessage(dwError));

        CString sErrorCode;
        sErrorCode.Format(_T("%u"), dwError);

        AfxFormatString2(sMsg, MapResourceID(IDS_NTSERV_VERBOSE_ERROR_STRING), sErrorCode, sDescription);
        break;
      }
      default:
      {
  ...
  else
  {
    switch (m_UILoggingDetail)
    {
      ...
      case UI_StringAndErrorCodeAndErrorDescription:
      {
        ...
        AfxFormatString2(sErrorAndDescription, MapResourceID(IDS_NTSERV_VERBOSE_ERROR_STRING), sError, sDescription);
