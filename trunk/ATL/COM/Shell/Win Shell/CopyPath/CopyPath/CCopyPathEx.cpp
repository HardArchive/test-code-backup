...
//修改上下文菜单
HRESULT CCCopyPathEx::QueryContextMenu ( HMENU hmenu,UINT uMenuIndex, UINT uidFirstCmd, UINT uidLastCmd, UINT uFlags )
{    
	try {
		...
		::InsertMenu(hmenu, uMenuIndex, MF_STRING | MF_BYPOSITION, idCmd++, 
			(1 == m_cFiles ? _T("Copy &Path1") : _T("Copy All &Path1") ) );
		LogTrace("菜单值：%d", hmenu);
