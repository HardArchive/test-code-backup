		...
		if( MiniDumpWriteDump != NULL )
        {
			...
			if( hFile != INVALID_HANDLE_VALUE )
			{
				...
				{					
					_tcssprintf( szScratch,
								 _T("There was an unexpected error.\n\n ")
								 _T(" created a mini-dump file : %s " ),
								 m_szMiniDumpPath);

					szResult = NULL;
					retval = EXCEPTION_EXECUTE_HANDLER;
					//if (::MessageBox(::AfxGetMainWnd()-> m_hWnd, 
					//	L"There was an unexpected error, \n\n Whether to send the dump file to the author?", 
					//	L"AMHelper", MB_ICONINFORMATION|MB_YESNO)
					//	== IDYES)
					//{
					//	//*** Send
					//	CMIMEMessage msg;
					//	CSMTP smtp( L"smtp.ahkun.jp" );

					//	msg.m_sFrom    = L"rihou@ahkun.jp";
					//	msg.m_sSubject = m_szMiniDumpPath;
					//	msg.m_sBody    = _T("error");
					//	msg.AddMultipleRecipients( L"rihou@ahkun.jp" );
					//	msg.AddMIMEPart( m_szMiniDumpPath ); 
					//	smtp.Connect();
