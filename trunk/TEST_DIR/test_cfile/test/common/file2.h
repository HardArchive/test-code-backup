...
* 6、 History    ： 去掉了对 string的依赖  改用TCHAR处理字符串
* 7、 Remark     ： 支持宽窄字符
****************************************************************************************************/
...
#define MAX_BUFFER_LEN 1024*4    //一次最大读写长度
    ...
    public:
        ...
        {
            m_fp = NULL;
			ZeroMemory(m_tszFileName, sizeof(m_tszFileName));
        }
       // CFile(const tstring &filename, const tstring &mode = _T("r"));//打开指定的文件 （可能会失败，使用 IsOpened())
		CFile(const TCHAR* ptszFileName, const TCHAR* ptszMode = _T("r"));
        CFile(FILE *lfp)
        {
        ...
        //打开/关闭
        bool Open(const TCHAR* ptszFileName, const TCHAR* ptszMode = _T("r"));//打开一个文件(如果不存在-由mode控制）
        bool Close();  //关闭已经打开的文件 （如果未打开，这是 NOP）

        //指定一个现有的文件描述符并取得 CFile 对象
        void Attach(FILE *lfp, const TCHAR* ptszFileName = _T(""))
        {
            Close();
            m_fp = lfp;
			_tcscpy_s(m_tszFileName, MAX_PATH, ptszFileName);
        }
        ...
        bool Write(TCHAR* ptszBuf);	                          //写放成功返回true
        bool Flush();                                         //刷新未写入的数据
        ...
        const void GetName(TCHAR* ptszFileName) const
        {
			_tcscpy_s(ptszFileName, _tcslen(m_tszFileName), m_tszFileName);
        }
    ...
    CFile::CFile(const TCHAR* ptszFileName, const TCHAR* ptszMode)
    {
        Detach();

        (void)Open(ptszFileName, ptszMode);
    }


    bool CFile::Open(const TCHAR* ptszFileName, const TCHAR* ptszMode)
    {
        int iRet = 0;
		m_fp = NULL;
		ZeroMemory(m_tszFileName, sizeof(m_tszFileName));
        //iRet = _tfopen_s(&m_fp, ptszFileName, ptszMode);
		m_fp = _tfopen(ptszFileName, ptszMode);
    ...
    bool CFile::Write(TCHAR* ptszBuf)
    {
		size_t size = 0;
        BYTE szbybuf[MAX_BUFFER_LEN] = {0};
		memset(szbybuf, 0, MAX_BUFFER_LEN);
		memcpy(szbybuf, ptszBuf, _tcslen(ptszBuf)*sizeof(TCHAR));
